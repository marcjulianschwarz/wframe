...
export function StoreDetail() {
  ...
  if (error) {
    return (
      <div className="p-ui-m rounded-ui-n border border-ui-border bg-ui-danger-bg text-ui-danger text-ui-s">
        {error}
      </div>
    );
  }
  if (!item) return <p className="text-ui-secondary text-ui-s">Loading…</p>;

  return (
    <section className="flex flex-col gap-ui-m">
      <Link to="/store" className="text-ui-s text-ui-secondary hover:text-ui-primary w-fit">
        ← Back to store
      </Link>
      <div className="flex gap-ui-l items-start flex-col lg:flex-row">
        <div className="flex flex-col gap-ui-m flex-1 w-full">
          <div>
            <h2>{item.title}</h2>
            <p className="text-ui-secondary text-ui-s mt-ui-xs">{item.description}</p>
          </div>
        ...
        <div className="flex flex-col gap-ui-s w-full lg:w-auto lg:shrink-0">
          <h2>Preview</h2>
